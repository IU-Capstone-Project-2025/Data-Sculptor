...
def run_all_linters(py_path: str) -> List[Dict]:
    ...
    with tempfile.TemporaryDirectory() as outdir:
        ...
        if os.path.exists(report_path):
            with open(report_path, encoding="utf-8") as rep:
                lines = [ln.rstrip("\n") for ln in rep]
            ...
            while i < len(lines):
                ...
                if line.startswith("- "):
                    ...
                    for ex in extra:
                        ex_norm = ex.strip()
                        if ex_norm.lower().startswith("location:"):
                            m_line = re.search(r"Location:\s*Line\s+(\d+)", ex_norm, re.I)
                            if m_line:
                                line_no = int(m_line.group(1)) - 1
                                break
                            m_tuple = re.search(r"\(\s*(\d+)\s*,\s*(\d+)\s*\)", ex_norm)
                            if m_tuple:
                    ...
                    if line_no == 0 and col_no == 0:
                        ...
                        line_no, col_no = find_position(py_path, smell_title, kw_list)
