...
class Message(BaseModel):
    ...
    def to_langchain(self) -> BaseMessage:
        if self.role == "system":
    ***REMOVED*** SystemMessage(content=self.content)
        if self.role == "assistant":
    ***REMOVED*** AIMessage(content=self.content)
***REMOVED*** HumanMessage(content=self.content)

    @staticmethod
    def to_langchain_list(messages: list["Message"]) -> list[BaseMessage]:  # noqa: D401
***REMOVED*** [m.to_langchain() for m in messages]
...
***REMOVED*** f"{line_part}: {self.message}"
