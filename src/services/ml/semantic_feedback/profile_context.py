    ...
    async def get_section(
        ...
        async with self._pg_pool.acquire() as conn:
            row = await conn.fetchrow(
                """
                SELECT pd.description AS profile_desc,
                       ps.description AS section_desc,
                       ps.code AS section_code
                FROM profile_sections ps
                JOIN profile_descriptions pd ON ps.case_id = pd.case_id
                WHERE ps.case_id = $1 AND ps.section_id = $2
                """,
                case_id,
                section_index,
            )
...
***REMOVED*** row["profile_desc"], row["section_desc"], row["section_code"]
